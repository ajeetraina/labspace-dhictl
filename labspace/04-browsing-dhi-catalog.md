...
First, tell the lab which organization to use:

::variableDefinition[dhiorg]{prompt="What is your DHI-enabled Docker Hub organization?"}

:::conditionalDisplay{variable="dhiorg" hasNoValue}
> [!WARNING]
> Set your organization above so the commands below are filled in for you.
:::

Then export your credentials in the terminal (replace the placeholder with your real token):

```bash no-run-button
export DHI_API_TOKEN="dckr_pat_xxxxxxxxxxxxxxxxxxxx"
export DHI_ORG="$$dhiorg$$"
```
