...
```bash no-run-button
docker dhi mirror start --org $$dhiorg$$ -r dhi.io/node,$$dhiorg$$/dhi-node
```

Check what's mirrored, and stop one when you're done:

```bash no-run-button
docker dhi mirror list --org $$dhiorg$$
```

```bash no-run-button
docker dhi mirror stop --org $$dhiorg$$ dhi-node
```
...
1. Generate a starting customization file from the base image:
    ...
    ```bash no-run-button
    docker dhi customization prepare --org $$dhiorg$$ node 20 --destination $$dhiorg$$/dhi-node-custom
    ```
...
3. Create the customization (which kicks off a build):
    ...
    ```bash no-run-button
    docker dhi customization create --org $$dhiorg$$ my-customization.yaml
    ```

4. Watch the build and read its logs:

    ```bash no-run-button
    docker dhi customization build list --org $$dhiorg$$ <customization-id>
    ```

    ```bash no-run-button
    docker dhi customization build logs --org $$dhiorg$$ <customization-id> <build-id>
    ```
